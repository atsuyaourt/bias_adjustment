...
plt_args = {
    ...
    "c_mod": {"name": "Model hist", "color": "red", "linetype": "-"},
    "c_mod_adj": {"name": "Model hist CDF", "color": "darkred", "linetype": "--"},
    "p_mod": {"name": "Model future", "color": "blue", "linetype": "-"},
    "p_mod_adj_cdf": {"name": "Model future CDF", "color": "teal", "linetype": "--"},
    "p_mod_adj_edcdf": {
        "name": "Model future EDCDF",
        "color": "deepskyblue",
        "linetype": "--",
    },
    "p_mod_adj_dqm": {"name": "Model future DQM", "color": "teal", "linetype": ":"},
    "p_mod_adj_qdm": {
...
dats = {
    "obs": {"dat": obs_df.loc[slice(*rf_yrs),]},
    "c_mod": {"dat": mod_df.loc[slice(*rf_yrs),]},
    "p_mod": {"dat": mod_df.loc[slice(*proj_yrs),]},
}

# region default
c_mod_adj_df = []
for m in range(1, 13):
    _obs_df = dats["obs"]["dat"].loc[
        (slice(None), m),
    ]
    _c_mod_df = dats["c_mod"]["dat"].loc[(slice(None), m),].copy()
    _c_mod_adj_df = _c_mod_df.copy()
    _c_mod_adj_df["val"] = do_qmap(
        _obs_df["val"].to_numpy(), _c_mod_df["val"].to_numpy()
    )
...
for adj_type in adj_types:
    ...
    for m in range(1, 13):
        ...
        _, _p_mod_adj_df["val"] = do_qmap(
            ...
            _c_mod_df["val"].to_numpy(),
            _p_mod_df["val"].to_numpy(),
            adj_type=adj_type,
        )
